...
> 分支：linux-support | 时间：2026-08-17（单日） | 提交链：15 个（含 1 merge，口径 `git rev-list --first-parent 5b43c5f..0202d58`；upstream 144 提交经 c71c96f 并入不计入）
...
### A. 跨渠道委派修复
- **v0.16.82**（c05a65e，链外背景·前份 PR 已覆盖）：跨渠道未传 modelId 自动选目标渠道默认模型（pickDefaultModelForChannel）
- **v0.16.84**（5b43c5f，基线起点）：startDelegation 运行渠道误传 ctx.channelId（父渠道端点+子渠道模型名→1214）。本地 CONNECT 代理抓包实锤根因
...
### D. Upstream 合并
- **v0.17.27**（c71c96f merge + 5c83cc1）：origin/main（144 提交，v0.17.42 baseline；执行者报告 463 为其环境时点数，当前实测 526）
...
| 交互确认 | 用户亲手勾选（21:45，21:30 事故后二次尝试成功） | ✅ |
| 强制 open_preview | e2e 两次触发 | ✅ |
| 中断保护 | 代码级修复+审查，修复后未复跑 e2e（回归测试缺） | ✅* |
| Upstream merge | 全量 526 pass/0 fail（复跑 2026-08-17 深夜，排除 1 例 Electron 依赖）+ M3 视觉 UI 回归 5/5（e2f74d8 message + /tmp/uitest-*.png） | ✅ BrowserPanel/侧栏/南大/ErrorBoundary/冒烟 |
| 对话式迭代 | **未 E2E**（待下一轮） | ⏳ |
| 导航规范 | **未 E2E**（待下一轮新原型） | ⏳ |
...
7. **deepseek 委派 1214 残余间歇故障**（status 5.3 记录：channelId 修复后 v4-flash 偶发 1214、probe 成功，黑盒待查）——验证矩阵 ✅ 仅指 channelId 修复本身
8. 视觉裁决与作者同模型（当时唯一视觉模型约束，靠证据隔离防自证）；open_preview 无路径授权校验（对比 resolveHtmlPreviewPath，纵深防御债待接入 fileAccess 体系）
...
测试：nanju-router(+18)/nanju-router-prompt(+14)/agent-preview-notify(+5)/agent-model-selection(+4)
