# 南大向导 v2.2 架构设计：两层固定路由 Harness（AC 审计收敛版）

> 基于 v2.0/v2.1 设计 + AC Round 1 审计修正（4 red + 8 yellow）
...
const ROUTES: Record<'quick' | 'iterative', PhaseNode[]> = {
  ...
  iterative: [
    // requirements 和 prototype 同 quick
    // prototype.next = 'architecture'（而非 'delivered'）
    {
      id: 'architecture',
      role: 'architect',
      title: '架构师',
...
function handlePhaseResult(result: PhaseResult, phase: PhaseNode): PhaseAction {
  ...
  if ((result.status === 'failed' || result.status === 'timeout') 
    ...
    return { action: 'retry', reason: result.error ?? 'L2 子会话失败' }
  }
  // 重试超限 → 通知用户
...
const PHASE_TOOL_WHITELIST: Record<PhaseId, Set<string>> = {
  requirements: new Set([
    ...
    'Read', 'LS', 'AskUserQuestion', 'EnterPlanMode', 'ExitPlanMode',
  ]),
  prototype: new Set([
    'delegate_agent', 'delegate_agents', 'wait_for_delegations',
    'list_delegations', 'stop_delegation',
...
每完成一个阶段后：
1. 将本阶段的 AC 审计结果写入文件（`{projectDir}/_ac-audit-{phaseId}.md`）
