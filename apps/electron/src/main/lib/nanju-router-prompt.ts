  ...
  const prompt = [
    ...
    ...(stage === 'prototype'
      ? [
        ...
        '   a. 【必须】先调用 open_preview 工具（file_path=' + projectDir + '/' + phase.outputPath + '）',
        '      确保右侧分屏正在展示可交互原型（文件监听可能已关，不要假设面板还开着）。',
        '   b. 再 Read ' + projectDir + '/01_PRD/prd.md，提取全部用户故事（US-xx）清单。',
        '   c. 提醒用户：「右侧预览面板已展示可交互原型（支持点击/输入/页内导航），请逐条实际操作验证」。',
        '   d. AskUserQuestion：header「原型交互验证」，multiSelect=true，',
        '      options = 每个用户故事一项（label=US-xx 简短标题，description=该故事的验收要点）',
        '                + 最后一项「全部通过，交付」；question 写明「请实际操作后勾选验证通过的用户故事」。',
        '   e. 用户勾选「全部通过」或选中所有故事 → 确认通过，进入第 5 步。',
        '   f. 有未勾选的故事 → 把未通过项 + 用户反馈整理成修改清单，用 continue_delegation',
        '      发给 UX 顾问子会话修复；修复后重新执行本步骤（重新 open_preview + AskUserQuestion）。',
      ]
      : [
