...
export function buildL2TaskWithAC(
  ...
  if (isTesting) {
    ...
    parts.push('· 页面刷新/重载（When 保存后刷新页面数据仍在/重新加载应用）→ {type:"reload"}（可选 selector：刷新后等待某元素出现，如 {type:"reload", selector:"…"}；数据持久化类故事用「填写→操作→reload→断言仍在」模式）。')
...
function buildAutoClarifyProtocolLines(): string[] {
  return [
    ...
    "4. 代理降级（fallback:'auto-degrade'——处理中被关/预算尽/熔断/无通道/无法作答）→ 登记 pendingQuestionIds",
    '   + 继续本阶段工作，产物附 <!-- auto-clarify:skipped,reason,ts -->，不得转述真人',
    '   （不因降级发起 AskUser 问用户）。',
    "4b. 【解除悬空 blocked（硬性）】fallback:'auto-degrade' 后禁止放任 blocked 悬空等待：",
    '   - 优先用保守自判（维持现状/最小改动倾向）经 answer_delegation_question 回注解除 blocked；',
    '   - 无法给出保守答案则 stop_delegation 终止该委派，并在产出记录 <!-- auto-clarify:skipped,terminated,ts -->。',
