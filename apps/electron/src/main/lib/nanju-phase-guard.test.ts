 ...
 *
 * 覆盖：计数（fail/error 独立累计）/ 阈值判定（fail≥2 或 error≥1 → open）/
...
describe('NANJU_GUARDS 配置口径（不写死：常量集中可调）', () => {
  test('软超时 1200s / 硬超时 2100s / 轮询 60s / fail 阈值 2 / error 阈值 1', () => {
    ...
    expect(NANJU_GUARDS.phaseFailBreakThreshold).toBe(2)
...
describe('isPhaseGuardCircuitOpen 阈值判定', () => {
  ...
  test('failCount=1 → closed；failCount≥2 → open（reason 含失败累计）', () => {
    expect(isPhaseGuardCircuitOpen({ failCount: 1, errorCount: 0 }).open).toBe(false)
    const opened = isPhaseGuardCircuitOpen({ failCount: 2, errorCount: 0 })
    expect(opened.open).toBe(true)
    expect(opened.reason).toContain('失败累计 2 次')
  })
  ...
  test('熔断全链路：两次 fail 轮次后状态从 closed → open（previous/current 支持新熔断检测）', () => {
    setupProject()
    ...
    const justOpened = !isPhaseGuardCircuitOpen(second.previous ?? undefined).open
      && isPhaseGuardCircuitOpen(second.current).open
